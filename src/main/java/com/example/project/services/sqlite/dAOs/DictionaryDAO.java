...
public class DictionaryDAO
{
    ...
    public String getWordDefinition(String wordToFind)
    {
        try
        {
            PreparedStatement query = connection.prepareStatement("SELECT word, wordtype, definition FROM combined_entries " +
                    "WHERE word =" +
                    " ? LIMIT 1");
    ...
    public boolean isWordInDictionary(String wordToCheck)
    {
        ...
        try
        {
            PreparedStatement doesWordExistQuery = connection.prepareStatement("SELECT 1 FROM combined_entries WHERE word = ? LIMIT 1");
